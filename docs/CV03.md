...
- Upravte třídu `UserTable` z předchozího cvičení na databázovou entitu.
...
- Upravte backend tak aby při startu přidal admin uživatele do databáze.
  ...
  - Třída bude dědit z `CommandLineRunner`.
    - Přepište metodu `run`:
      - Vytvořte novou instanci třídy `UserTable` tak aby reprezentovala admin uživatele.
...
Vymyslete další entitu, která bude v libovolném vztahu s `UserTable` a přidejte jí do projektu. Pokud Vás nic nenapadá využijte AI.

1. Třída musí mít alespoň 1 atribut, který nesouvisí s relací.
2. Vytvořte relaci pomocí anotací s nově vytvořenou třídou `UserTable`. Využijte (`@OneToOne`, `@OneToMany`, `@ManyToMany`)
3. Vložte alespoň jeden nový záznam do tabulky, který bude ve vztahu s existujícím záznamem z tabulky `UserTable`.
